...
time docker run \
   ...
   -it nixnote2/xenial \
   /bin/bash -c "cd nixnote2 && git fetch && git checkout $PROJECTBRANCH && git pull && ./development/build-with-qmake.sh debug noclean /opt/tidy56 && ./development/create-AppImage.sh && mv *.AppImage appdir && chmod -R a+rwx appdir"
