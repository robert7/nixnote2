...
void StartupConfig::printHelp() {
    QString help = QString("usage: " NN_APP_NAME " <command>\n"
                           ...
                           + QString(
        "  start <options>                      Start NixNote GUI with the specified options.\n")
                           + QString(
        "                                       If no command is specified, this is the default.\n")
                           + QString("     start options:\n")
                           + QString(
        "          --logLevel=<level>           Set initial logging level (0=trace,1=debug,2=info,3=error,..\n")
                           + QString(
        "                                       This is ONLY valid at program startup until settings are read.\n")
                           ...
                           + QString(
        "                                       Warning: ff you set configDir, but don't set userDataDir; userDataDir defaults\n")
                           + QString("                                       to configDir.\n")
                           + QString(
        "          --programDataDir=<dir>       Directory containing deployed fixed program data (like images).\n")

                           + QString("          --dontStartMinimized         Override option to start minimized.\n")
                           + QString("          --disableEditing             Disable note editing\n")
                           + QString(
        "          --enableIndexing             Enable background Indexing (can cause problems)\n")
        ...
        "  sync                                 Synchronize with Evernote without showing GUI.\n")
                           ...
                           + QString(
        "  show_window                          If running, ask NixNote to show the main window.\n")
                           + QString(
        "  query <options>                      If running, search NixNote and display the results.\n")
                           + QString("     query options:\n")
                           + QString("          --search=\"search string\"     Search string.\n\n")
                           + QString(
        "          --delimiter=\"character\"      Character to place between fields.  Defaults to |.\n")
                           ...
                           + QString(
        "                <padding>              Pad the field to this number of spaces on the display.\n")
                           + QString(
        "                <:>                    Truncate the field if longer than the padding.\n")
                           ...
                           + QString(
        "                                       For multiple tags use multiple --tag statements.\n")
                           + QString("          --attachment=\"<file_path>\"   File to attach to the note.\n")
                           + QString(
        "                                       For multiple files, use multiple --attachment statements.\n")
                           + QString(
        "          --delimiter=\"<delmiiter>\"    Character string identifying attachment points.\n")
                           + QString("                                       Defaults to %%.\n")
                           + QString(
        "          --created=\"<datetime>\"   Date & time created in yyyy-MM-ddTHH:mm:ss.zzzZ format.\n")
                           + QString(
        "          --updated=\"<datetime>\"   Date & time updated in yyyy-MM-ddTHH:mm:ss.zzzZ format.\n")
                           + QString(
        "          --reminder=\"<datetime>\"  Reminder date & time in yyyy-MM-ddTHH:mm:ss.zzzZ format.\n")
                           + QString(
        ...
        "          --id=\"<note_ids>\"            Space separated list of note IDs to extract.\n")
                           ...
                           + QString(
        "          --reminder=\"<datetime>\"      Set a reminder in yyyy-MM-ddTHH:mm:ss.zzzZ format")
                           + QString("          --reminderClear              Clear the note's reminder.\n")
                           + QString("          --reminderComplete           Set the reminder as complete.\n")
                           + QString(
        "                                       yyyy-MM-ddTHH:mm:ss.zzzZ format or the literal 'now' to default\n")
                           ...
                           + QString(
        "  deleteNote <options>                 Move a note to the trash via the command line.\n")
        ...
        "  emailNote <options>                  Move a note to the trash via the command line.\n")
                           ...
                           + QString(
        "          --bcc=\"<address list>\"       List of recipients to blind carbon copy.\n")
        ...
        "          --openNote                   Open a note.  --id=<id> must be specified.\n")
                           + QString(
        "          --openNoteNewTab             Open a note in a new tab.  --id=<id> must be specified.\n")
                           + QString(
        "          --openExternalNote           Open a note in an external window.  --id=<id> must be specified.\n")
